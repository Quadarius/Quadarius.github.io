...
/***********************
* I'm Quadarius Moore *
* This is Click the blocks to break them into smaller blocks  *
***********************/
...
class blockContainer {
    constructor(blocklist) {
        ...
        this._size = blocklist.length;
    }

    findBlock(x, y){
        console.log('finding')
        for (var i = 0; i < this._size; i++) {
            let block = this._blockList[i];

            if(block.x <= x && block.y <= y && (block.x+block.size) >= x && (block.y+block.size) >= y){
                return block;
            }
        }
    }
    ...
    splitBlock(b){
        let blocklevel = b.level;

        if(blocklevel < 4 ){
            b.shrink();
            b.level = blocklevel + 1;

            this._blockList.push(new block(b.x+b.size, b.y, b.size, getColor(), blocklevel + 1))
            this._size = this._size + 1;
            this._blockList.push(new block(b.x+b.size, b.y+b.size, b.size, getColor(), blocklevel + 1))
            this._size = this._size + 1;
            this._blockList.push(new block(b.x, b.y+b.size, b.size, getColor(), blocklevel + 1))
            this._size = this._size + 1;
            fraps += 0.05;
        }
    ...
    removeBlock(block){
        this._blockList.remove(block)
    }
}
class block {
    constructor(x, y, size, color, level = 0){
        this._x = x;
        this._y = y;
        this._size = size;
        this._color = color;
...
////////////////////////////////////////////////////////////////////////////////
var bs = [];
var b = new block(0,0,800,startColor);
bs.push(b);
var blocks = new blockContainer(bs);
...
function mouseClicked(){
    var clickedBlock = blocks.findBlock(mouseX, mouseY);
    blocks.splitBlock(clickedBlock);
    frameRate(fraps);
    redraw(fraps);
    //drawBlocks(blocks.blocks);
}
